...
Y.namespace('M.atto_a11lyaxe').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    initializer: function() {
        ...
        Y.Get.js(M.cfg.wwwroot + '/lib/editor/atto/plugins/a11lyaxe/amd/src/axe.js', function(err) {
            if (err) {
                Y.log('Could not load axe-core.js');
            }
        });
    },
                                ...
                                buttons: {
                                    Close: function() {
                                        ...
                                        $.ajax({
                                            ...
                                            // Error handling
                                            Y.log('Failed update stats');
                                        }).done(function () {
                                            // Success msg
                                            Y.log('Successfully update stats');
                                        });
                                    },
                                    Prev: function() {
                                        if (errorIndex > 0) {
                                    ...
                                    Focus: function() {
                                        if (result.length > 0){
                                            ...
                                            $(this).remove();
                                            state = 0;
                                            Y.log(result[errorIndex].nodes[0].target);
                                            errorElement = result[errorIndex].nodes[0].target.toString();
                                            Y.log(document.querySelectorAll(errorElement));
